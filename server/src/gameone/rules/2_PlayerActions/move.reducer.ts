...
export const MoveReducer: StateReducer<MoveAction> = (state, action) => {
  ...
  if (state.players[state.currentPlayerIndex] !== action.playerId) {
    ...
    throw new Error(`non è il tuo turno! (tocca al giocatore con ID <${state.players[state.currentPlayerIndex]}>)`);
  ...
  if (myTile.enemiesIn.length >= myTile.playersIn.length) throw new Error("il giocatore non si può muovere perché il numero dei giocatori nella tile non supera quello dei nemici");
  ...
  if (
    ...
    throw new Error("il giocatore non si può muovere perché ha finito il numero dei movimenti");
  ...
  if (nodeInDirection === undefined) {
    ...
    throw new Error("il giocatore non si può muovere in quella direzione perché non c'è un passaggio");
  ...
  if (nodeInDirection === null) {
    ...
    myTile.playersIn = myTile.playersIn.filter((pid) => pid !== action.playerId);
