...
export const PickATileReducer: StateReducer<PickATileAction> = (
  ...
  if (nodeInDirection === undefined) {
    ...
    throw new Error("la tile non può essere posizionata perché non c'è un passaggio in quella direzione");

  }
  if (nodeInDirection !== null) {

    throw new Error("la tile non si può posizionare in quella direzione perché già occupata");

  }
  const tileCoord = getNeighborhoodCoord(
    myTile,
    action.position,
  );
