...
function rotateTile (tile: ConnectedTile, rotation: TileAngles): ConnectedTile {
  ...
  if (Object.values(tile.nodes).some((val) => val != null)) {
    ...
    throw Error("La rotazione deve avvenire solo per tile preparate ma non ancora collegate!");

  }
  // metto i nodi in un array
  const nodes: { pos: Connections;
    node: Tile | null | undefined; }[] = [
    { pos: "TL",
      node: tile.nodes.TL },
    { pos: "TR",
      node: tile.nodes.TR },
  ...
  const rotatedNodes = nodes.slice(
    rotation,
    rotation + 6,
  );
  tile.nodes = {};
