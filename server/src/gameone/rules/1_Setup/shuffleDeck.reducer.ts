...
export const ShuffleTilesReducer: StateReducer<ShuffleTilesAction> = (
  ...
  if (state.boss == null) throw new Error("Non posso mischiare le tile perchè il boss non è stato scelto");
  const tiles1: Tile[] = shuffleArray(OTHER_TILES.filter((tile) => tile.ring === 1));
  const tiles2: Tile[] = shuffleArray(OTHER_TILES.filter((tile) => tile.ring === 2));
  const bossTile = shuffleArray(BOSS_TILES)[0];
  const tiles3: Tile[] = shuffleArray(OTHER_TILES.filter((tile) => tile.ring === 3).concat([bossTile]));
  return {
