...
describe("Match Making controller", () => {
  ...
  it("Get a list of free games with empty list", async () => {
    const games = await mmController.getGames();
    expect(games.length).toBe(0);
  });

  it("Get a list of free games after create one", async () => {
  ...
  it("Get a list of free games after create one, join with other 3 players", async () => {
    const gameId = await mmController.createGame({
      gameTitle: "Test",
      nickName: "player 1",
    });
    // fill the game
    await Promise.all([
      mmController.joinGame({ nickName: "player 2", gameId }),
      mmController.joinGame({ nickName: "player 3", gameId }),
      mmController.joinGame({ nickName: "player 4", gameId }),
    ]);
    const games = await mmController.getGames();
  ...
  it("Start a game", async () => {
    ...
    await mmController.startGame({ gameId, nickName: "player 1" });
